...
2. 볼트에 요약본 1개 생성:
   - 파일명: `{YYYY-MM-DD}_{slug}.md` (**날짜 = 원본 문서 생성일**, 수집일 아님 / slug = 영문 kebab 주제)
   - **생성일 산출 우선순위**: ① 원본 본문에 명시된 `작성일` → ② 없으면 git 최초 커밋일(`git log --diff-filter=A --follow --format=%ad --date=short -- <원본>`의 마지막 줄). 여러 원본을 통합한 요약본은 대표(앵커) 문서 기준일 + 필요 시 구성 문서 날짜 범위를 괄호 병기. (git 최초 커밋일이 레포 시딩일이면 그 이전 실제 작성일은 알 수 없으므로 시딩일을 가용 최선값으로 사용)
   - 형식 (기존 raw/career 문서와 통일):
     ```
     # {프로젝트/주제} — 원본 자료

     작성일: {YYYY-MM-DD}   # 원본 문서 생성일
     출처: outputs/{원본 경로들}
...
- **같은 슬러그의 기존 요약본이 있을 때**: 덮어쓰지 말고 해당 파일 하단에 `## {YYYY-MM-DD} 추가 — {원본 경로}` 섹션을 append한다. 단, 동일 원본이 이미 출처 라인에 명시되어 있으면 스킵하고 사용자에게 알린다. **파일명 날짜(원본 문서 생성일)는 그대로 유지** — append할 때 새 날짜로 바꾸지 않는다.
