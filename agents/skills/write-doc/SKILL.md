...
- **내용은 기획자가 작성하는 수준의 톤으로 작성합니다.** 코드/시스템 관점의 압축 표현이 아니라, 행위자(관리자·유저·운영자 등) 단위로 행위·조건·결과를 풀어서 기술합니다.
  - **배경·구현(요구사항) 섹션은 보고서체(개조식)로 종결합니다.** 문장 끝을 `~한다.`/`~된다.`/`~이다.`/`~본다.`/`~있다.`가 아니라 **명사·어간으로** 맺습니다. 예: "상세지역을 고르면 상위 도시를 자동으로 채워 **보여준다**. 다른 시/도의 구가 함께 선택되면 저장을 막고 **안내한다**." → "상세지역을 고르면 상위 도시를 자동으로 채워 **노출**. 다른 시/도의 구가 함께 선택되면 저장을 막고 **안내**". (행위자·조건·결과 구조는 유지하고 **마지막 서술어만** 명사형/어간형으로.)
