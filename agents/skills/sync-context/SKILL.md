...
> **반영 범위 — 당일 전량**: 이번 세션에서 만든 문서뿐 아니라, **당일(오늘) 생성·수정·삭제된 모든 문서와 변경사항**을 빠짐없이 GitHub에 반영한다. 다른 세션·워크스페이스에서 편집된 대기 변경(신규·수정·삭제)도 포함한다. `git status`에 잡힌 변경은 원칙적으로 전부 커밋 대상. 단, 의도치 않은 변경(오타·`.DS_Store`·실수 편집)이나 이유 불명 삭제는 Step 2에서 짚고 사용자 확인 후 포함/제외한다.
