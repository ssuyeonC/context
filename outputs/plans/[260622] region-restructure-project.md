...
> ⚠️ **일부 모델은 후속 문서가 정본** — 본 문서(Path 1)의 개념 모델 중 **CITY=시도(L1) · DETAIL_LOCATION=시군구(L2, LEGAL 참조) · REGION 스팟 해상도=legal(법정동) 기반 · 정비=LEGAL 정합·비파괴**는 `[260701] geo-model-decision §0`·`master`가 최신 정본이다. 본 문서의 고유 정본은 **작업분해(D1~D10·Phase/BE/FE/AD)**. 개념 서술이 마스터와 다르면 마스터를 따른다.
...
| D7 | **테마 = REGION 섹션(자유) + master_theme 사전** | REGION 섹션 = 이름+카테고리+순서(자유 편성), REGION 섹션 CMS에서 master_theme에 매핑. **CITY 집계는 B-2**(CITY 스팟 ∩ 테마, 축은 home-nav §5-2), REGION 그룹핑 불필요 |
...
- **페이지 내 테마 섹션 = 캐러셀** (city·zone 공통): '더보기→리스트' 폐기(→ 이전 'W3: 더보기→리스트 흡수' 결정 **갱신**). **monthly best 상위 9개**(기존 컴포넌트 가정, 디자인 시 조정) 캐러셀 + 스팟이 더 있으면 **"전체 보기" → 해당 leaf**.
  ...
  - 스팟 = **city×테마: CITY 스팟 ∩ 테마(B-2) / zone×테마: REGION 선택 법정동 ∩ 테마**. zone×테마는 카테고리 2개 이상 묶을 수 있어 `/spot/list` 단일필터로 표현 불가 → leaf 전용 쿼리. 데이터는 기존 그대로, **FE 페이지 템플릿 + 라우팅만 추가.**
...
- CITY 콘텐츠 입력 + 활성화 (image·tags·desc)
...
**닫힌 결정 (2026-06-24)**
- ✅ CITY 콘텐츠 저장소 → **카드 메타만 저장**(별도 콘텐츠 테이블 없음). 상세 스팟 = **CITY 스팟 전체 ∩ theme (B-2)**
